...
/*   Updated: 2021/09/02 17:03:06 by anaouadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*append_line(char *line, char *remainder, int sline, char *ptr)
{
	...
	if (!line)
		line_re = ft_strncpy(remainder, sline_index(remainder) + 1);
	else
		line_re = ft_strjoin(line, remainder, sline_index(remainder) + 1);
	if (ptr != NULL)
		free(ptr);
	return (line_re);
}
...
char	*put_remainder(char *remainder, int sline_index, int alloc, int ret)
{
	...
	if (!str)
		return (NULL);
	ft_bzero(str, len(remainder) - sline_index + 1);
	while (remainder[i])
	{
		str[j] = remainder[i++];
		j++;
